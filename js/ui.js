    ...
    el.innerHTML = `
      ...
      <div class="rc-body">
        ...
        <div class="rc-rating">
          ...
          <span class="rc-num">${v.rating > 0 ? v.rating.toFixed(1) : 'N/A'}${v.reviews ? ' ('+v.reviews+')' : ''}</span>
        </div>
...
function showDetail(placeId) {
  ...
  App.placesService.getDetails({
    ...
    fields: ['name','rating','user_ratings_total','formatted_phone_number','website',
             'opening_hours','photos','formatted_address','price_level']
  }, (place, st) => {
    ...
    const reviewsBlock  = totalReviews > 0 ? `
      <div class="reviews-cta">
        ...
        <a class="reviews-cta-btn"
           href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(place.name+' '+(place.formatted_address||''))}&query_place_id=${placeId}"
           target="_blank" rel="noopener">📖 Leggi tutte le recensioni</a>
      </div>`
      : '<div style="opacity:.5;font-size:14px">Nessuna recensione disponibile</div>';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // inserisce search-tools subito dopo .search-bar-wrap
  const wrap = document.querySelector('.search-bar-wrap');
  if (wrap && !document.querySelector('.search-tools')) {
    wrap.insertAdjacentHTML('afterend', `
      <div class="search-tools">
        <button id="btn-filtro" class="tool-btn filtro"
                aria-expanded="false" aria-controls="vibe-filters">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
            <path d="M3 5h18l-7 8v6l-4 2v-8L3 5z" stroke="currentColor" stroke-width="2"/>
          </svg>
          Vibe
        </button>
        <button id="btn-extra" class="tool-btn ghost" aria-label="Scelta casuale">🎲</button>
      </div>
    `);
  }

  $('btn-filtro')?.addEventListener('click', () => {
    const panel   = $('vibe-filters');
    const chips   = $('chips');
    const isOpen  = panel.classList.toggle('open');
    chips.classList.toggle('chips-open', isOpen);
    $('btn-filtro').setAttribute('aria-expanded', String(isOpen));
    toast(isOpen ? `${EMOJI.sparkle} Filtra per vibe` : '✕ Filtri chiusi');
  });

  $('btn-extra')?.addEventListener('click', () => {
    // reset
    ACTIVE_VIBES.cucina  = [];
    ACTIVE_VIBES.vibe    = [];
    ACTIVE_VIBES.prezzo  = null;
    ACTIVE_VIBES.discover = [];

    // sceglie cucina + vibe casuali
    const cucinaItems = ORBO_VIBES.cucina.items;
    const vibeItems   = ORBO_VIBES.vibe.items;
    const rc = cucinaItems[Math.floor(Math.random() * cucinaItems.length)];
    const rv = vibeItems[Math.floor(Math.random() * vibeItems.length)];
    ACTIVE_VIBES.cucina = [rc.id];
    ACTIVE_VIBES.vibe   = [rv.id];

    renderChips();
    renderVibeChips('vibe-filters');
    searchAPI(buildSearchQuery(''));
    toast(`${EMOJI.trending} ${rc.emoji} ${rc.label} + ${rv.emoji} ${rv.label}`);
  });
});